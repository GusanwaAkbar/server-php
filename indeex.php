...
<body>
<a href="?page=home">Home</a> | <a href="?page=tambah">Tambah Data</a> | <a href="?page=daftar-data">Data Server</a> <br/><br/>
<fieldset>
<? if ($_GET['page']'tambah') { ?>
<legend>Tambah Data</legend>
<form name="form" method="POST" action="proses.php"> <input type="hidden" name="aksi" value="tambah"/>
...
?>
$r Sabc-Stampil_data($_GET['id_barang']);
<legend>Ubah Data</legend>
<form name="form" method="post" action="proses.php">
<input type="hidden" name="aksi" value="ubah"/>
<input type="hidden" name="id_barang" value="<?=$r->barang->id_barang?>" /> <label>ID Barang</label>
<input type="text" value="<?=$r->barang->id_barang?>" disabled>
<br/>
<label>Nana Barang</label>
<input type="text" name="nama barang" value="<?=$r->barang->nama_barang?>"> <br/>
<button type="submit" name="ubah">Ubah</button> </form>
<? unset($r);
?>
...
$no = 1;
$data_array $abc->tampil semua_data();
foreach ($data array as $r) {
?> <tr><td><?=$no?></td>
<td><?=$r->id_barang?></td>
<td><?=$r->nama_barang?></td>
<td><a href="?page=ubah&id barang=<?=$r->id_barang?>">Ubah</a></td>
