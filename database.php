...
class Database
{
    private $host="localhost"; private $dbname="toko";
    private $user="root";
    ...
    {
        $query = $this->conn->prepare("insert ignore into barang (id_barang, nama_barang) values (?,?)"); $query->execute(array($data['id_barang'], $data['nama_barang']));
        $query->closeCursor();
        unset($data);
    }
    ...
    { 
        $query = $this->conn->prepare("update barang set nama barang=? where id_barang=?");
    ...
    {
        $query = $this->conn->prepare("delete from barang where id barang=?");
        $query->execute(array($id_barang));
        $query->closeCursor();
        unset($id_barang);
    }
